...
- `artifacts/ai-native-office/src/components/WhitepaperBody.tsx` — the shared section renderer (`SectionBody`): all prose/list/table/math/code/margin-note rendering (with `renderText` → `tokenizeCitations` so inline source numbers become `<Citation>` markers), plus the inline EgressCalculator/ArchitectureBlueprint widgets. Used by both Home and SectionPage; heading level and appendix-link hrefs are props.
