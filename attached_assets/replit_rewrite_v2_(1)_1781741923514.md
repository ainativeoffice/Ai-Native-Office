# ainativeoffice.org — Full Page Structural Rewrite
## Complete verbatim replacement copy for all six main sections.
## Drop each section in exactly as written. Do not paraphrase, reformat, or reorder.
## Preserve: site title, subtitle, byline, nav, Works Cited, footer, cost calculator, icons, and all CSS.

---

## OVERVIEW OF CHANGES

The page is being restructured from its current section order into a clean six-section narrative arc:

1. The Structural Ceiling (replaces Introduction + absorbed "why now" content)
2. Who This Is For (promoted from near-bottom to second position)
3. How the Architecture Works (replaces Sovereign Enclave + Sensory Organ, stripped of hardware specs)
4. The Economics of Sovereignty (replaces Cloud Egress Trap — preserves the cost calculator in place)
5. The Compliance Moat (replaces Intelligence Flywheel — stripped of GraphRAG pipeline mechanics)
6. Engage (replaces Implementation Path + existing Engagement Paths)

The current nav links point to section IDs. Update the nav anchor labels and IDs to match the new section headings as follows:
- `#intro` → `#ceiling` / label: "The Structural Ceiling"
- `#egress` → `#for` / label: "Who This Is For"  
- `#sensory` → `#architecture` / label: "How It Works"
- `#enclave` → `#economics` / label: "The Economics"
- `#flywheel` → `#compliance` / label: "The Compliance Moat"
- `#conclusion` → `#engage` / label: "Engage"

---

## SECTION 1 — THE STRUCTURAL CEILING

**REPLIT INSTRUCTION:** Find the section currently with id="intro" (heading: "Introduction: The Architecture of the Next Layer"). Replace the section id with `id="ceiling"`. Replace the section heading and ALL body paragraphs with the following. Preserve the section wrapper element and any existing CSS classes on it.
...
**REPLIT INSTRUCTION:** Find the section currently with id="egress" (heading: "The Cloud Egress Trap"). Replace the section id with `id="for"`. Replace the section heading and ALL body content — including all sub-sections, the bandwidth math paragraphs, the LiveKit benchmarks, the egress pricing table, and the cost calculator — with the following prose. EXCEPTION: Preserve the cost calculator component exactly where it is — do NOT remove it. Position it after the fourth paragraph of this section (after the paragraph ending "...resolves the stall."), before the transition line.
...
[COST CALCULATOR — PRESERVE THE EXISTING INTERACTIVE CALCULATOR COMPONENT EXACTLY HERE. Do not remove it. Do not replace it. It already exists on the page.]

---

## SECTION 3 — HOW THE ARCHITECTURE WORKS

**REPLIT INSTRUCTION:** Find the section currently with id="sensory" (heading: "The Space as a Sensory Organ"). Replace the section id with `id="architecture"`. Replace the section heading and ALL body content — including all acoustic telemetry sub-sections, Shure MXA920 specs, Dante/AES67 detail, Asterisk PBX paragraphs, Casambi BLE mesh sections, and spatial tracking content — with the following. Preserve the section wrapper and CSS classes.
...
## SECTION 4 — THE ECONOMICS OF SOVEREIGNTY

**REPLIT INSTRUCTION:** Find the section currently with id="enclave" (heading: "The Sovereign Enclave"). Replace the section id with `id="economics"`. Replace the section heading and ALL body content — including the STC 55 construction table, staggered stud wall specs, door assembly detail, dark fiber paragraphs, and the full compute class specification — with the following. The existing interactive cost calculator has already been repositioned to Section 2 (above). Do not duplicate it here.
...
## SECTION 5 — THE COMPLIANCE MOAT

**REPLIT INSTRUCTION:** Find the section currently with id="flywheel" (heading: "The Intelligence Flywheel & Absolute Sovereignty"). Replace the section id with `id="compliance"`. Replace the section heading and ALL body content — including the GraphRAG pipeline description, entity extraction detail, Leiden algorithm paragraphs, and vector indexing content — with the following. Preserve the section wrapper and CSS classes.
...
**REPLIT INSTRUCTION:** Find the section currently with id="conclusion" (nav label: "The Implementation Path"). Replace the section id with `id="engage"`. Replace the section heading and ALL body content — including the McLuhan paragraph, the four-bullet summary, and the existing engagement paths — with the following. Preserve the "Initialize a Conversation" contact block and its existing styling; update only the text within it as shown.
...
### CONTACT BLOCK (replace existing "Initialize a Conversation" content):
...
1. Do not modify: site title ("The Room as the Machine"), subtitle, byline, Works Cited section, site footer, nav structure (only update anchor labels and IDs as specified above), cost calculator component, icon placements, or any CSS.

2. The existing technical sections — bandwidth math, sensor hardware specs, compute class specifications, GraphRAG pipeline, STC construction tables — are not deleted permanently. Move them to placeholder sub-pages or comment them out in the source, so they can be restored as linked technical appendices later.

3. After implementing all six sections, verify that:
   - All six nav links resolve correctly to the new section IDs
   - The cost calculator remains functional and positioned after the fourth paragraph of Section 2
   - The four blockquote principles in Section 3 render with the same visual treatment as the existing blockquotes on the page
   - No section heading or body paragraph from the old structure remains visible on the page
